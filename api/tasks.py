...
from .models import RegistredUser, Order, ProductItem, Producer, Category
...
def get_producer_statistic():
    ...
    with open('producer_statistic.txt', 'w') as stat_file:
        for key, value in products_stat.items():
            product = ProductItem.objects.get(id=int(key))
            producer = Producer.objects.get(id=int(product.producer))
            producer_stat[producer.id] += int(value)

        for key, value in producer_stat.items():
            producer = Producer.objects.get(id=int(key))
            stat_text = f"Producer {producer.name} made {value} orders\n"
            stat_file.write(stat_text)
...
def get_category_statistic():
    ...
    with open('category_statistic.txt', 'w') as stat_file:
        for key, value in products_stat.items():
            product = ProductItem.objects.get(id=int(key))
            category = Category.objects.get(id=int(product.category))
            category_stat[category.id] += int(value)

        for key, value in category_stat.items():
            category = Category.objects.get(id=int(key))
            stat_text = f'Category {category.name} had {value} orders'
            stat_file.write(stat_text)
